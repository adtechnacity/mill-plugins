...
./mill duplication                   # Workspace-wide copy-paste detection
...
`coverage-html` artifact), the mutation score of the PR's delta — every module with a changed `.scala` file is
run through the repo's own published `mill-stryker4s` plugin, with the HTML reports in the `mutation-html` artifact —
and the copy-paste detection summary of the dogfooded `mill-cpd` (`./mill duplication`). CI also runs the example
workspaces.
